...
class SkFighter:
    def __init__(self, player, x, y, flipped, data, sprite_list, animation_steps, sound):
        self.player = player
        self.size = data[0]
        self.scale = data[1]
        self.img_offset = data[2]
        self.animation_list = self.load_sprites(sprite_list, animation_steps)
        self.flipped = flipped
        self.action = 0  # 0:idle 1:walk 2:jump 3:attack1&2 4:hit 5:death
        self.frame_index = 0
    ...
    def load_sprites(self, sprite_list, animation_steps):
        animation_list = []
        for y, animation in enumerate(animation_steps):
            tmp_img_list = []
            for x in range(animation):
                tmp_img = sprite_list[y].subsurface(x * self.size, 0, self.size, self.size)
